...
df_long["Emotional state"] = df_long["Emotional state"].str.replace("I feel: ", "").str.strip()
...
for version in df_long["Version"].dropna().unique():
    ...
    plt.title(f"Average Change in Emotional State After Interview — {version}")
    plt.xticks(rotation=45)
    plt.axhline(0, color='black', linestyle='--')
    plt.tight_layout()
    filename = f"emotional_state_change_{version}.png".replace(" ", "_")
