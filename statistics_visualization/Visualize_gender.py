...
df_long = df.melt(
    id_vars=["Which version of the robot were you interviewed by?", "Before/After", "What is your gender?"],
...
#Create graph for each version and gender
for version in df_long["Which version of the robot were you interviewed by?"].unique():
    for gender in df_long["What is your gender?"].dropna().unique():
        subset = df_long[
            (df_long["Which version of the robot were you interviewed by?"] == version) &
            (df_long["What is your gender?"] == gender)
        ]
        ...
        plt.title(f"{gender} – Mean Moods before and after interview with {version}")
